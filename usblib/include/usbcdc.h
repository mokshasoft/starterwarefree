...
#define INT(pucData)           (*(unsigned int *)(pucData))
...
#define GetRingerExists(pcData)  ((INT(pcData)) & USB_CDC_RINGER_EXISTS)
...
#define SetResponseCallState(pcBuf, ucIndex, ulActive, ucStateChange,   \
        ...
        do                                                              \
        {                                                               \
            (INT((unsigned char *)(pcBuf) + (10 + (4 * (ucIndex))))) = \
...
#define SetNotifyConnectionSpeedChange(pcBuf, ulUSBitRate, ulDSBitRate) \
        do                                                              \
        {                                                               \
            INT(pcBuf) = ulUSBitRate;                                  \
            INT((unsigned char *)(pcBuf) + 4) = ulDSBitRate;           \
        }                                                               \
        while(0)
